...
func Scan(target string, skip []string, noSpinner bool) ([]string, time.Duration, int) {
	...
	for i, tool := range models.Tools {
		...
		if contains(skip, tool.Name) || !checkTool(strings.Split(tool.Command, " ")[0]) {
			...
			fmt.Println(models.Yellow("Scanning Tool Unavailable. Skipping Test...") + models.Reset())
			skippedChecks++
			continue
		}

		fmt.Printf("\n[%s%s] Deploying %d/%d | %s%s%s\n", tool.Status.ProcLevel, tool.Status.EstTime, i+1, len(models.Tools), models.Blue(""), tool.Description, models.Reset())
		tempFile := fmt.Sprintf("/tmp/vulny_temp_%s", tool.Status.ID)
		...
		if err != nil && err != exec.ErrNotFound {
			...
			fmt.Println(models.Yellow("Test Skipped. Press Ctrl+C to quit Vulny.") + models.Reset())
			skippedChecks++
			if !noSpinner {
				spinner.Start()
			}
	...
	fmt.Println(models.Blue("[ Preliminary Scan Phase Completed. ]") + models.Reset())
...
func runTool(cmdStr string, target string, tempFile string) (string, time.Duration, error) {
	...
	output, errRead := os.ReadFile(tempFile)
	if errRead != nil {
...
func printVulInfo(tool models.Tool) {
	fmt.Printf("%sVulnerability Threat Level%s\n", color.New(color.Bold).Sprint(""), models.Reset())
	fmt.Printf("\t%s %s%s\n", models.VulInfo(tool.Resp.Severity), models.Yellow(tool.Resp.Message), models.Reset())
	fmt.Printf("%sVulnerability Definition%s\n", color.New(color.Bold).Sprint(""), models.Reset())
	remedIndex := tool.Resp.RemedIndex - 1
	if remedIndex >= 0 && remedIndex < len(models.Remediations) {
		fmt.Printf("\t%s%s\n", models.Red(models.Remediations[remedIndex].Description), models.Reset())
		fmt.Printf("%sVulnerability Remediation%s\n", color.New(color.Bold).Sprint(""), models.Reset())
		fmt.Printf("\t%s%s\n", models.Green(models.Remediations[remedIndex].Remediation), models.Reset())
	} else {
		fmt.Printf("\t%sNo remediation information available.%s\n", models.Red(""), models.Reset())
	}
}