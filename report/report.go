...
func GenerateReport(target string, vulnerabilities []string, totalChecks, skippedChecks int, totalElapsed time.Duration) {
	fmt.Println(models.Blue("[ Report Generation Phase Initiated. ]"))

	currentDate := time.Now().Format("2006-01-02")
	vulReport := fmt.Sprintf("rs.vul.%s.%s", target, currentDate)
	debugLog := fmt.Sprintf("rs.dbg.%s.%s", target, currentDate)

	if len(vulnerabilities) == 0 {
		fmt.Println(models.Green("No Vulnerabilities Detected."))
	} else {
		...
		for _, vul := range vulnerabilities {
			...
			data, _ := ioutil.ReadFile(fmt.Sprintf("/tmp/rapidscan_temp_%s", parts[0]))
			f.Write(data)
			f.WriteString("\n\n")
		}
	...
	for _, tool := range models.Tools {
		...
		data, err := ioutil.ReadFile(fmt.Sprintf("/tmp/rapidscan_temp_%s", tool.Status.ID))
		if err == nil {
	...
	fmt.Println(models.Blue("[ Report Generation Phase Completed. ]"))

	// Clean up temporary files
	files, _ := filepath.Glob("/tmp/rapidscan_temp_*")
